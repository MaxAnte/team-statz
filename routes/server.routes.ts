...
import PlayoffsMatchup from "../models/PlayoffsMatchup";

import { User as UserType } from "../client/src/context/session.types";
import { DateType } from "../client/src/context/app.types";
import { Game as GameType } from "../client/src/context/app.types";
import { Team as TeamType } from "../client/src/context/app.types";
import { Settings as SettingsType } from "../client/src/context/app.types";
...
router.post(
  "/date/add-date",
  [],
  async (req: CustomRequest<DateType>, res: Response) => {
    try {
...
router.post(
  ...
  async (req: CustomRequest<Partial<GameType>>, res: Response) => {
    try {
      ...
      game.pending = false;
      if (ourScore) game.ourScore = ourScore;
      if (enemyScore) game.enemyScore = enemyScore;
      if (quarters) game.quarters = quarters;

      const enemyWin = enemyScore && ourScore ? +enemyScore > +ourScore : false;

      const enemyTeam = await Team.findOne({ name: enemy });
      if (!enemyTeam)
        return res.status(400).json({ message: `Cant find ${enemy} team` });
      const ourTeam = await Team.findOne({ name: config.get("TEAMNAME") });
      if (!ourTeam)
        return res
          .status(400)
          .json({ message: `Cant find ${config.get("TEAMNAME")} team` });
      ...
      if (playersStats) {
        ...
        playersStats.forEach(async (player) => {
          ...
          if (!playerDB)
            return res
              .status(400)
              .json({ message: `${player._id}: stats not found` });
...
router.post(
  ...
  async (req: CustomRequest<Partial<GameType>>, res: Response) => {
    try {
      ...
      const enemyWin =
        (!!enemyScore ? +enemyScore : game.enemyScore) >
        (!!ourScore ? +ourScore : game.ourScore);
      const enemyTeam = await Team.findOne({ name: enemy });
      if (!enemyTeam)
        return res.status(400).json({ message: `Cant find ${enemy} team` });
      const ourTeam = await Team.findOne({ name: config.get("TEAMNAME") });
        ...
        playersStats.forEach(async (player, i) => {
          ...
          if (i === 0) {
            game?.playersStats.forEach(async (p) => {
              if (
                !playersStats.find(
                  (pl) => pl._id.toString() === p._id.toString()
                )
              ) {
...
router.post(
  "/game/delete-game",
  [],
  async (req: { body: { _id: string } }, res: Response) => {
    try {
...
router.post("/bracket/build", [], async (req: Request, res: Response) => {
  ...
  const teams = await Team.find({});
  if (!teams) return res.status(400).json({ message: `Cant gett teams` });
...
// /api/game/edit-game
// router.post("/players/clean", [], async (req: Request, res: Response) => {
//   try {
//     const players = await Player.find({});
//     if (!players) return res.status(400).json({ message: `Players not found` });
